...
WHY HTTP: profiles are validated HTTP resources with their own identity
and status codes. Profile writes are client-side only and never touch
any render path (M3 removed the last one).
"""
...
class AdminLinkProfileApi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        response = self.client.post("/api/admin/nodes", json={
            "id": self.node, "label": "Tokyo 01", "address": "funky.example.com",
        })
        assert response.status_code == 201, response.text
        put = self.client.put(f"/api/admin/nodes/{self.node}/config",
                              json=_config())
        assert put.status_code == 200, put.text

    def test_create_get_update_delete_roundtrip(self):
        created = self.client.post(
            f"/api/admin/nodes/{self.node}/link-profiles", json=_profile()
        )

        self.assertEqual(created.status_code, 201)
        self.assertEqual(created.json()["id"], "cdn")
        self.assertEqual(created.json()["inbound_tag"], "xhttp")
        listed = self.client.get(f"/api/admin/nodes/{self.node}/link-profiles")
        self.assertEqual([p["id"] for p in listed.json()], ["cdn"])
        fetched = self.client.get(
            f"/api/admin/nodes/{self.node}/link-profiles/cdn")
        self.assertEqual(fetched.json()["label"], "CDN")

        updated = self.client.put(
            f"/api/admin/nodes/{self.node}/link-profiles/cdn",
            json={"label": "CDN Edge", "overrides": {
        ...
        self.assertEqual(
            self.client.get(
                f"/api/admin/nodes/{self.node}/link-profiles/cdn").status_code,
            404,
        )
    ...
    def test_profile_writes_are_client_side_only(self):
        ...
        self.client.put(
            f"/api/admin/nodes/{self.node}/link-profiles/cdn",
            json={"label": "CDN Edge"})
        self.client.delete(f"/api/admin/nodes/{self.node}/link-profiles/cdn")
    ...
    def test_duplicate_unknown_and_missing_profiles(self):
        ...
        unknown_inbound = self.client.post(
            f"/api/admin/nodes/{self.node}/link-profiles",
            json=_profile("other", "ghost")
        )
        ghost_node = self.client.post(
            "/api/admin/nodes/ghost/link-profiles", json=_profile()
        )
        missing = self.client.get(
            f"/api/admin/nodes/{self.node}/link-profiles/ghost")
